...
function FillInBlank( arrSents, numBlanks, topXWords ) {
    ...
    this.hashWordIndex = FillInBlank.indexWordList( this.topUCW );
    
    //TEST::
    console.log( "hashWordList:\n", this.hashWordIndex );
...
FillInBlank.prototype = {
    ...
    selectWordForBlank: function( sent ) {
        ...
        //replace each word with a "_____"
        var regExRandWord = new RegExp( randWord, 'gi'  );
        return {'sentBlank': sent.replace( regExRandWord, "_____"), 'answer': randWord};
        // return {'sentBlank': sent.replace( randWord, "<span name = 'blank'>_____</span>"), 'answer': randWord};
    },

    createFillInBlanks: function() {
        //METHOD: create a fill-in-the-blank sentence
        var uniqWords, getBlankInfo;
        var arrSentBlanks = [];     //
        ...
        for ( var i in this.arrSents ) {
            if ( (this.arrSents[i] === undefined) || !(/\S/.test(this.arrSents[i])) ) 
                continue
            getBlankInfo = this.selectWordForBlank( this.arrSents[i] );
            arrSentBlanks.push( getBlankInfo['sentBlank'] );
            arrAnswers.push( getBlankInfo['answer'] );
        }
